...
**Active Vehicle (state):** Zustand store `useActiveVehicleStore` (`entities/vehicle/store`), persisted to localStorage (`odolog:active-vehicle`), holds `activeVehicleId`. **URL is the source of truth** — `useSyncVehicleIdFromUrl()` mirrors `:vehicleId` from URL into the store on vehicle-scoped pages. `useResolvedVehicleId()` — returns active vehicle ID if it exists in DB, otherwise falls back to first vehicle and syncs the store; used on pages without `vehicleId` in URL (`/settings`). Active Vehicle is device-local — not part of the sync protocol. See `docs/adr/0001-active-vehicle-url-vs-store.md`. **Zustand** (`zustand` v5) is the app's client-state library.
