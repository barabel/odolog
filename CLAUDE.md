...
**Drawers (bottom sheets):** Imperative mechanism in `shared/lib/drawer` over `vaul`. `useDrawer()` gives `openDrawer(key, props?, options?)` / `closeDrawer()`; `<DrawerProvider registry={registry}>` is mounted **inside `BrowserRouter`** (so sheet content can use router hooks) wrapping `Routes` in `App.tsx`. `useDrawer()` also gives `closeAll()` (closes the whole stack). Stack lives in a pure `drawerStackReducer(state, action) → Frame[]` (`open`/`close`/`closeAll`/`remove`; unit-tested) — vaul callbacks only dispatch. **Closing is two-phase:** `close` flips the last **open** frame's (`findLastIndex(frame.open)`) `open:false`, vaul plays the exit animation, then a `setTimeout(EXIT_ANIMATION_MS = 550ms)` in `DrawerFrame` (keyed on `frame.open` going false) dispatches `remove` (`closeAll` flips `open:false` on every frame; each is then removed by its own timer). The timer is used — not vaul's `onAnimationEnd` — because with a controlled `open` prop Radix's `useControllableState` fires `onChange`/`onAnimationEnd` only on vaul-initiated closes (gesture/Esc/overlay), not on programmatic ones (close button, `closeDrawer()`, `closeAll()`), so relying on it would leave programmatically-closed frames stuck in the stack. Enter animation: each frame mounts with local `open:false` and flips to `true` in an effect so vaul sees the false→true transition. **Nesting:** `DrawerStack` renders the stack **recursively** — frame 0 → `Drawer.Root`, deeper frames → `Drawer.NestedRoot` rendered as a JSX child of the parent's `Drawer.Content` (vaul needs that parent-context nesting to scale the parent), while each level's own `Drawer.Portal` teleports its content to `body`. Depth is unbounded. Sheet content lives in `widgets/drawers/<key>/ui` as a **plain component free of any `Drawer.*`** — chrome (handle, close button) comes from `DrawerStack`. Register a sheet: add `key → { component, options? }` to `widgets/drawers/registry.tsx` and augment `interface DrawerRegistryMap` there (gives `openDrawer` key autocomplete + compile error on typos). Contexts are split — `useDrawer()` reads only the stable actions context, so triggers don't re-render when the stack changes. **Per-drawer options (`DrawerOptions`, a subset of vaul `DialogProps`: `snapPoints`/`dismissible`/`direction`/`modal`):** a sheet sets defaults once via `registry[key].options`; `openDrawer(key, props, options)` overrides per-call. The two are shallow-merged by pure `mergeDrawerOptions(base?, override?)` (`shared/lib/drawer/model`, unit-tested) into `frame.options`, which `DrawerStack` spreads onto `Drawer.Root`/`NestedRoot` (`{...frame.options}`) — so e.g. `dismissible: false` makes gesture/overlay/Esc no-ops while `closeDrawer()`/the close button still work.
