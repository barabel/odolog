...
**Drawers (bottom sheets):** механизм удалён — был на `vaul`, который ломается с экранной клавиатурой на мобилках (шит не опускается назад). Пишется свой с нуля. Контент шитов сохранён: `widgets/drawers/<key>/ui` (`DrawerOdometer`, `DrawerFuel`) — обычные компоненты, сейчас нигде не смонтированы.
