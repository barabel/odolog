...
**Drawers (bottom sheets):** Imperative mechanism in `shared/lib/drawer` over `vaul`. `useDrawer()` gives `openDrawer(key, props?, options?)` / `closeDrawer()`; `<DrawerProvider registry={registry}>` is mounted **inside `BrowserRouter`** (so sheet content can use router hooks) wrapping `Routes` in `App.tsx`. Stack lives in a pure `drawerStackReducer(state, action) → Frame[]` (`open`/`close`/`remove`; unit-tested) — vaul callbacks only dispatch. **Closing is two-phase:** `close` flips the top frame's `open:false`, vaul plays the exit animation, then `onAnimationEnd(false)` dispatches `remove`. Enter animation: each frame mounts with local `open:false` and flips to `true` in an effect so vaul sees the false→true transition. Sheet content lives in `widgets/drawers/<key>/ui` as a **plain component free of any `Drawer.*`** — chrome (handle, close button) comes from `DrawerStack`. Register a sheet: add `key → { component, options? }` to `widgets/drawers/registry.tsx` and augment `interface DrawerRegistryMap` there (gives `openDrawer` key autocomplete + compile error on typos). Contexts are split — `useDrawer()` reads only the stable actions context, so triggers don't re-render when the stack changes. Nesting (`NestedRoot`, `closeAll`) and per-call option override are later slices.
