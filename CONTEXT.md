...
| **Entry** | Запись на таймлайне одометра: показание одометра в указанный пользователем момент времени плюс опциональная нагрузка по `type`. Можно создавать сколько угодно, в т.ч. задним числом. Разновидности — Odometer entry, Fuel entry. |
| **Odometer entry** | Тип Entry (`type: 'odometer'`): только показание одометра. |
| **Fuel entry** | Тип Entry (`type: 'fuel'`): показание одометра + литры + общая стоимость заправки. |
| **Mileage** | Пробег — разница одометра между двумя соседними Entry (любых типов) по времени измерения. |
