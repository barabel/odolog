...
- Каждая запись имеет `id` (UUID, генерируется на клиенте) и `updated_at` (timestamp ms)
- Локальный флаг `synced: boolean`
- Push: отправляем все записи с `synced = false`
- Pull: тянем с сервера всё новее `last_sync_at`
- Конфликт по `id`: побеждает запись с бо́льшим `updated_at`
